...
# Reference: https://epubs.siam.org/doi/10.1137/080716542
...
def l1_yogi(M, v, mu, L, verbose=False):
    N = L*L
    assert M.shape == (N,N)
    assert v.shape == (N,)
    c = jnp.zeros((N,))
    ...
    def loss(c):
        c = SCALE*c
        d = jnp.fft.rfft2(c.reshape((L,L)))
        return c.T @ M @ c - 2 * c @ v + mu*jnp.sum(jnp.abs(d))
...
def soft_threshold(beta, lamda):
    return (beta - lamda)*(beta > lamda) + (beta + lamda)*(beta < -lamda)

def l1_proximal(M, v, mu, L, verbose=False):
    N = L*L
    assert M.shape == (N,N)
    assert v.shape == (N,)

    t = 1e-3
    # We do things directly in Fourier space, on d.
    c = jnp.zeros((N,))
    d = jnp.fft.rfft2(c.reshape((L,L)))

    def loss(d):
        c = jnp.fft.irfft2(d).reshape((N,))
        return c.T @ M @ c - 2 * c @ v

    @jax.jit
    def update(d):
        lval, grad = jax.value_and_grad(loss)(d)
        d = soft_threshold(d - t*grad, t*mu)
        return lval, d
...
if __name__ == '__main__':
    ...
    if False:
        ...
        def basis_at(idx):
            x, y = idx%model.L, idx//model.L
            phi_t = jnp.roll(phi_, (x,y), axis=(1,2))
            dS_t = jnp.roll(dS_, (x,y), axis=(1,2))
            return basis_(phi_t, dS_t)
        basis = jnp.sum(jax.lax.map(basis_at, jnp.arange(N)), axis=0)
    else:
        def basis_add(b, idx):
            x, y = idx%model.L, idx//model.L
            phi_t = jnp.roll(phi_, (x,y), axis=(1,2))
            dS_t = jnp.roll(dS_, (x,y), axis=(1,2))
            return b + basis_(phi_t, dS_t), None
        basis = 0*basis_(phi, dS)
        basis, _ = jax.lax.scan(basis_add, basis, jnp.arange(N))
    ...
    for dt in tqdm(dts):
        ...
        c = l1_solve(M, v, args.mu, args.L)
        #print(c.T@M@c - 2*v@c + v@v, np.sum(np.abs(c) > 1e-6), np.sum(np.abs(c) > 1e-7))
        #print(c.T@M@c - 2*v@c + v@v)
        d = jnp.fft.rfft2(c.reshape((L,L)))
        print(np.min(np.abs(d)), np.max(np.abs(d)), np.sum(np.abs(d) > 1e-6), file=sys.stderr)
